<template>
  <section class="p-2 flex justify-center">
    <div class="auth-form w-full lg:w-1/3 p-2">
      ...
      />
      <!-- {{ authStor?.userDetailData?.code }}
      {{ authStor?.state?.login }} -->
      <div
        v-if="!authStor?.userDetailData?.code"
        class="getMobile flex items-center flex-col align-center"
      >
        <h1>{{ $t("login") }}</h1>
        <p class="text-sm text-gray-400">{{ $t("loginMessage") }}</p>

        <input
          class="border border-gray-300 p-1 rounded w-full mt-2"
          type="text"
          @keyup="validateMobile(mobileNUm)"
          @keydown.enter=""
          v-model="mobileNUm"
...
"
      ...
      <div class="loginPassword flex items-center flex-col align-center hidden">
        ...
        <button
          class="border border-yellow-400 hover:bg-yellow-500 p-1 rounded w-full mt-3"
        >
          {{ $t("GetAone-timeCode") }}
        </button>
      </div>
      <!-- v-if="authStor?.userDetailData?.code == authStor?.state?.login" -->
      <div
        v-if="authStor?.userDetailData?.code == authStor?.state?.login"
      ...
      >
        ...
        <p class="text-sm text-gray-700">
          ...
          <button
            ...
            @click="authStor.userDetailData.code = null"
          >
        ...
        <div class="input-group flex flex-row-reverse justify-around w-full mt-2">
          <input
            class="border border-gray-300 p-1 rounded w-[50px] px-5"
            maxlength="1"
            type="text"
            @keyup="focusNext('inpu2')"
            v-model="pin1"
          />
          <input
            id="inpu2"
        ...
        <button
          @click="signIn(mobileNUm, pin1 + pin2 + pin3 + pin4)"
